...
export default function Home() {
  ...
  const sourceDates = [
    ...
    { label: "CRM", date: crmSummary.dataAsOfDate ?? "—" },
    { label: "Calendar", date: weekSummary.dataAsOf ?? "—" },
  ];
  const sourceTooltip = sourceDates.map((s) => `${s.label}: ${s.date}`).join(" · ");
          ...
          <div className="flex items-center gap-2.5">
            ...
            {lastUpdated && (
              <button
                ...
                title={isRefreshing ? "Refreshing data..." : `Click to refresh · ${sourceTooltip}`}
              >
                <RefreshCw
                  size={10}
